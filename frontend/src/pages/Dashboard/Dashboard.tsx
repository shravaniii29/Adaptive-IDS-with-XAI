...
export default function Dashboard() {
  ...
  const benignPackets = history.reduce((sum, entry) => sum + (!isAttack(entry.hybrid_prediction) ? (entry.packet_count || 0) : 0), 0);
  const attackPackets = history.reduce((sum, entry) => sum + (isAttack(entry.hybrid_prediction) ? (entry.packet_count || 0) : 0), 0);

...
export default function AgentInsightsCard({ result, prediction }: { result: AgentAnalysisResponse | null; prediction: Prediction | null }) {
  ...
  // Prefer the real backend agent output already returned by /predict
  // (response_agent/explainability_agent), falling back to the /analysis
  // route (if ever implemented) and finally the local heuristic.
  const severity = agent?.severity?.toUpperCase() || prediction?.threat_level || (prediction ? riskLevel(prediction) : "WAITING");
  const summary = agent?.soc_analyst_summary || agent?.explanation || prediction?.summary || (prediction ? plainSummary(prediction) : "The system is waiting for the first network flow.");
  const action = agent?.incident_report?.recommended_next_step || agent?.recommended_mitigation?.[0] || prediction?.recommended_action || (prediction ? nextAction(prediction) : "No action needed yet.");
